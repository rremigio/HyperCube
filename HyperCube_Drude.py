...
class ViewerWindow(QMainWindow):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_B:
            if not self.drude_active:
                x_cursor, y_cursor = self.spectrum_cursor_pos  # Get cursor position
                self.line_region = self.get_line_region(x_cursor)  # Check if cursor is in x1, x2 range
                if self.line_region is not None:
                    self.initial_x = x_cursor
                    self.current_gamma = 10
                    self.current_amplitude = y_cursor
                    
                    self.start_drude_adjustment(self.line_region['region_ID'])
            else:
                self.drude_active = False
                self.save_drude(self.line_region['region_ID'])
    ...
    def start_drude_adjustment(self, region_ID):
        if self.drude_active:
            self.drude_active = False
            return
        
        self.drude_active = True
        ...
        if self.spectrum_cursor_pos:
            self.drude_x0, _ = self.spectrum_cursor_pos
            
        self.drude_gamma = 10 * abs(wavelengths[1] - wavelengths[0])
        self.drude_amplitude = 0.1 * (np.diff(self.spectrum_ax.get_ylim())[0])
        
        self.drude_component_lines = []
        
        self.drude_active = True
        
        self.spectrum_canvas.mpl_connect("motion_notify_event", self.update_drude_dynamic)
    ...
    def update_gaussian_dynamic(self, event):
        ...
        for _, row in df.iterrows():
            p_type = str(row.get('Profile_Type', 'gaussian')).lower()
            if p_type == 'gaussian':
                y_gaussian = row["Amp_0"] * np.exp(-((x_vals - row["Centroid_0"]) ** 2) / (2 * row["Sigma_0"] ** 2))
            elif p_type == 'drude':
                y_gaussian = row["Amp_0"] * row["Sigma_0"]**2 / ( (x_vals / row["Centroid_0"] - row["Centroid_0"] / x_vals)**2 + row["Sigma_0"]**2)
    ...
    def update_drude_dynamic(self, event):
        if not self.drude_active or event.inaxes != self.spectrum_ax:
            return
        
        dx = event.xdata - self.drude_x0
        dy = event.ydata
        
        self.drude_gamma = max(0.0001, abs(dx)**(1.2))
        
        y_continuum_x0 = self.m * self.drude_x0 + self.b
        
        self.drude_amplitude = dy - y_continuum_x0
        
        x_vals = np.linspace(self.x1, self.x2, 1000)
        y_line = self.m * x_vals + self.b
        
        y_drude_total = np.zeros_like(x_vals)
        
        while self.drude_component_lines:
            line = self.drude_component_lines.pop()
            line.remove()
            
        for _, row in df.iterrows():
            p_type = str(row.get('Profile_Type', 'gaussian')).lower()
            if p_type == 'drude':
                y_drude = row["Amp_0"] * row["Sigma_0"] ** 2 / ( (x_vals / row["Centroid_0"] - row["Centroid_0"] / x_vals)**2 + row["Sigma_0"]**2 )
            elif p_type == 'gaussian':
                y_drude = row["Amp_0"] * np.exp(-((x_vals - row["Centroid_0"]) ** 2) / (2 * row["Sigma_0"] ** 2))
            y_drude_total += y_drude
            
            component_line, = self.spectrum_ax.plot(x_vals, y_drude + y_line, color='#d7801a', linestyle='--', linewidth=1)
            self.drude_component_lines.append(component_line)
        
        y_drude_new = self.drude_amplitude * self.drude_gamma**2 / ( (x_vals / self.drude_x0 - self.drude_x0 / x_vals)**2 + self.drude_gamma**2 )
        y_drude_total += y_drude_new
        
        new_component_line, = self.spectrum_ax.plot(x_vals, y_drude_new + y_line, color='#d7801a', linestyle='--', linewidth=1)
        self.drude_component_lines.append(new_component_line)
        
        y_new = y_line + y_drude_total
        self.active_line.set_data(x_vals, y_new)
        self.spectrum_ax.figure.canvas.draw_idle()
    ...
    def save_drude(self, region_ID):
        ...
        df_new = pd.DataFrame({'Line_ID': [line_id],
                ...
                'SNR': [0],
                'Rest Wavelength': [np.nan],
                'Amp_0': [self.drude_amplitude],
                'Amp_0_lowlim': [0],
                'Amp_0_highlim': [np.inf],
                'Centroid_0': [self.drude_x0],
                'Centroid_0_lowlim': [-np.inf],
                'Centroid_0_highlim': [np.inf],
                'Sigma_0': [self.drude_gamma],
                'Sigma_0_lowlim': [0],
                'Sigma_0_highlim': [np.inf],
                'Amp_fit': [np.nan],
                'Centroid_fit': [np.nan],
    ...
    def rebuild_plot(self,region_ID, from_file, show_init, show_fit, x, y):
        ...
        if show_init == True:
            ...
            # '''
            components = df.loc[np.int64(df["region_ID"]) == region_ID]
    
            # Recompute all Gaussians from scratch (prevents runaway summing)
            y_components_total = np.zeros_like(x_vals)
            ...
            for _, row in components.iterrows():
                ...
                p_type = str(row.get('Profile_Type', 'gaussian')).lower()
                if p_type == 'gaussian':
                    comp = row["Amp_0"] * np.exp(-((x_vals - row["Centroid_0"]) ** 2) / (2 * row["Sigma_0"] ** 2))
                elif p_type == 'drude':
                    comp = row["Amp_0"] * row["Sigma_0"]**2 / ( (x_vals / row["Centroid_0"] - row["Centroid_0"] / x_vals)** 2 + row["Sigma_0"]**2 )
                
                new_curve, = self.spectrum_ax.plot(x_vals, comp+y_line, color='#d7801a', lw=1,linestyle='--')
                df.loc[(np.int64(df['region_ID']==region_ID)) & (df['Line_ID'] == i), 'curveactor'] = new_curve
                y_components_total += comp
                i = i+1
                
        
            # Final y-values
            y_new = y_line + y_components_total
        ...
        if (show_fit == True) and (len(df_fit) > 0):
            ...
            for _, row in region.iterrows():
                ...
                p_type = str(row.get('Profile_Type', 'gaussian')).lower()
                if p_type == 'gaussian':
                    y_comp = row["amp_fit"] * np.exp(-((x_vals - row["cen_fit"]) ** 2) / (2 * row["sigma_fit"] ** 2))
                elif p_type == 'drude':
                    y_comp = row["amp_fit"] * row["sigma_fit"]**2 / ( (x_vals / row["cen_fit"] - row["cen_fit"] / x_vals)** 2 + row["sigma_fit"]**2 )
                
                y_gaussian_total += y_comp
                fit_component, = self.spectrum_ax.plot(x_vals,y_line+y_comp,lw=1,linestyle='--',color=plotcolor)
...
class FitParamsWindow(QtWidgets.QMainWindow):
    ...
    def load_cube_fit(self):
        ...
        if file_path:
            ...
            float_columns = [
                ...
                if col not in ["color", "success", "LineName", "LineID", "spaxel_x", "spaxel_y", "region_ID", 'Profile_Type']
            ]
    ...
    def fit_spaxel(self,z,max_nfev=256,params_to_use=None):
        ...
        try:
            err_flux = self.viewer_window.fits_data['Err_st']
            ...
            #Make dictionary where key = line and value = df of fit values 
            line_fits = {f"{line.Line_Name} {line.Line_ID}": pd.DataFrame(columns=['amp_fit', 'cen_fit', 'sigma_fit', 'vel_fit']) for line in df.itertuples()}
            #Make dictionary where key = cont_region_num and value = df of cont fit values
            cont_fits = {f'cont_region{i+1}': pd.DataFrame(columns=['slope_fit', 'intercept_fit']) for i in range(len(df_cont))}
            N = 100
            for i in range(N):
                random_flux = np.random.normal(loc=spectrum, scale=err_flux)
                random_flux = np.nan_to_num(random_flux)
                result = piecewise_model.fit(random_flux, params_to_use, x=wavelengths, max_nfev=max_nfev)
                ...
                for line_idx, line in enumerate(df.itertuples(), start=1):
                    ...
                    p_type = getattr(line, 'Profile_Type', 'gaussian')
                    ...
                    name =f"{line.Line_Name} {line_id}"
                    #Add one fit run to dataframe for each line
                    add_row = pd.DataFrame([[result.params[amp_key].value, result.params[cen_key].value, result.params[sigma_key].value, vel_fit, result.bic, result.redchi, result.success, p_type]], 
                                           columns=['amp_fit', 'cen_fit', 'sigma_fit', 'vel_fit', 'bic', 'rchisq', 'success', 'Profile_Type'])
                    line_fits[name] = pd.concat([line_fits[name], add_row], ignore_index=True)
                    
                    
                    
                for region_id in df_cont['region_ID'].unique():
                    region_index = df_cont[df_cont['region_ID'] == region_id].index[0] + 1
                    cont_name = f'cont_region{region_index}'
                    
                    # Capture main slope/intercept
                    slope = result.params[f'slope{region_index}'].value
                    intercept = result.params[f'intercept{region_index}'].value
                    
                    cont_row = {'slope_fit': slope, 'intercept_fit': intercept}
                    
                    # Capture intermediate slope if it exists
                    if f'slope_int_{region_index}' in result.params:
                        cont_row['slope_int_fit'] = result.params[f'slope_int_{region_index}'].value
                    
                    # Append to DataFrame
                    cont_fits[cont_name] = pd.concat([cont_fits[cont_name], pd.DataFrame([cont_row])], ignore_index=True)
            ...
            for line_idx, line in enumerate(df.itertuples(), start=1):
                ...
                cont_fit_df = cont_fits[f'cont_region{region_index}']
                ...
                cont_params = {
                    ...
                    f'cont_region{region_index}_slope_fit': np.median(cont_fit_df['slope_fit']),
                    f'cont_region{region_index}_intercept_init': params_to_use[f'intercept{region_index}'].init_value,
                    f'cont_region{region_index}_intercept_fit': np.median(cont_fit_df['intercept_fit'])
                }
                ...
                if region_index < len(df_cont):
                    cont_params.update({
                        ...
                        f'cont_region{region_index}_slope_int_fit': np.median(cont_fit_df['slope_int_fit']),
                    })
                ...
                name = f'{line_name} {line_id}'
                line_fit_df = line_fits[name]
                final_p_type = line_fit_df['Profile_Type'].iloc[0] if 'Profile_Type' in line_fit_df.columns else 'gaussian'
                ...
                fit_entry = {
                    ...
                    'LineName': line_name,
                    'LineID': line_id,
                    'Profile_Type': final_p_type,
            
                    # Amplitude information
                    'amp_init': params_to_use[amp_key].init_value,
                    'amp_fit': np.median(line_fit_df['amp_fit']),
                    'amp_std': np.std(line_fit_df['amp_fit']),
            
                    # Centroid information
                    'cen_init': params_to_use[cen_key].init_value,
                    'cen_fit': np.median(line_fit_df['cen_fit']),
                    'cen_std': np.std(line_fit_df['cen_fit']),
                    
                    # Velocity information
                    'vel_init': vel_init,
                    'vel_fit': np.median(line_fit_df['vel_fit']),
                    # 'vel_param': vel_param,
                    # 'vel_std': vel_std,
                    ...
                    'sigma_fit': np.median(line_fit_df['sigma_fit']),
                    'sigma_std': np.std(line_fit_df['sigma_fit']),
            
                    # Fit statistics
                    'BIC': np.median(line_fit_df['bic']),
                    'rchisq': np.median(line_fit_df['rchisq']),
                    'success': all(line_fit_df['success'])
                }
    ...
    def fit_cube(self, refit=False, rchisq_thresh=None):
        ...
        for i, row in df_cont.iterrows():
            ...
            for j, line in enumerate(df.itertuples(), start=1):
                ...
                #Add Profile Type 0 for Gaussian, 1 for Drude
                p_type_val = 1.0 if getattr(line, 'Profile_Type', 'gaussian').lower() == 'drude' else 0.0
                params.add(f'type{j}', p_type_val, vary=False)
